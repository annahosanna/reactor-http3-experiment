...
- Load testing has topped out at 500 connections per second.
...
- Please see SPA.md for updates on the SPA status.

## Summary

- The following methods are supported:
  - Get with `Content-Type` or `Accept` = `text/html` returns a regular web page
  - Get with `Content-Type` or `Accept` = `application/json` returns a JSON object of type `List<Map<String,String>>`
    - This only returns a single fortune
  - Get with `Content-Type` or `Accept` = `text/plain` returns a single fortune as plain text
  ...
  - Put with `Content-Type` = `application/json` expects a JSON object of type `List<Map<String,String>>`
    - Multiple fortunes can be added in a single request this way
- Note: Flux(null) is not valid, but otherwise null values are passed correctly internally.
- Note: This data structure cannot be mapped to a pojo because that relies on the keys having a specific name.
...
- PUT workflow passes the JSON string through Jackson to validate the data structure.
- The GET routes and non blocking servers are mostly derived from the examples below.
...
- Browser Testing
  ...
  - QUIC is especially sensitive to valid certificates, and will silently fail to negotiate - even in developer mode, browsers do not report HTTP/3 connection attempts
...
- QUIC (which http3 is on top of) and HTTP/3 has acutally been around for a while. There were a large number of drafts of each with different libraries supporting different draft versions. QUIC and HTTP/3 have been finalized and are no longer in draft.
...
- Breaking this down a bit further it means one piece of the program focuses on creating quic on top of udp, and a different part of the program focuses on creating http3 on top of quic.
